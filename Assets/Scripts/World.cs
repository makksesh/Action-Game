...
{
    public int mapSizeInChunks = 6; // максимальное количество чанков в мире
    public int chunkSize = 16, chunkHeight = 100; // размеры чанка
    public int waterThreshold = 50; // уровень воды
    public float noiseScale = 0.03f; // уроен шуме Перлина
    public GameObject chunkPrefab; 

    Dictionary<Vector3Int, ChunkData> chunkDataDictionary = new Dictionary<Vector3Int, ChunkData>();
    Dictionary<Vector3Int, ChunkRenderer> chunkDictionary = new Dictionary<Vector3Int, ChunkRenderer>();

    public Dictionary<Vector3Int, ChunkData> GetChunkData()
    {
    ...
    private void GenerateVoxels(ChunkData data)
    {
        for (int x = 0; x < data.chunkSize; x++)
        {
            for (int z = 0; z < data.chunkSize; z++)
            {
                ...
                int groundPosition = Mathf.RoundToInt(noiseValue * chunkHeight); // на этом уровне генерируется поверхность 
